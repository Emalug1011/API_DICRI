...
export async function crearIndicio(req, res) {
  const id_expediente = parseInt(req.params.id, 10);
  const { descripcion, color, tamano, peso, ubicacion } = req.body;
  const id_usuario_tecnico = 1; // Esto luego puede venir del token
  ...
  try {
    ...
    console.error(err);
    return res.status(500).json({ message: "Error al crear indicio", detail: err.message });
  }
}
